...
-- timginter @ GitHub
-- [seh] Adapted from https://github.com/tekezo/Karabiner/issues/814#issuecomment-535082008.
------------------------------------------------------------------------------------------

-- ID of mouse wheel button
local mouseScrollButtonId = 2                -- [seh] NB: Logi Options is sending middle click.

-- scroll speed and direction config
local scrollSpeedMultiplier = 0.08           -- [seh] Modified from 0.1.
local scrollSpeedSquareAcceleration = true
local reverseVerticalScrollDirection = true  -- [seh] Modified from false.
local mouseScrollTimerDelay = 0.01

-- circle config
local mouseScrollCircleRad = 7               -- [seh] Modified from 10.
local mouseScrollCircleDeadZone = 2          -- [seh] Modified from 5.
...
overrideScrollMouseUp = hs.eventtap.new({ hs.eventtap.event.types.otherMouseUp }, function(e)
    if e:getProperty(hs.eventtap.event.properties.mouseEventButtonNumber) == mouseScrollButtonId then
        ...
        if (xDiff < mouseScrollCircleDeadZone and yDiff < mouseScrollCircleDeadZone) and not mouseScrollCircle then
            ...
            hs.eventtap.keyStroke({}, "pageup")  -- [seh] Modified to send other key instead.
        ...
        return false -- [seh] This was true before.
    end
end)
