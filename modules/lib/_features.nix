...
{lib}: let
  ...
  # name", where "host" is the resolved "config.dotfiles._resolved"
  # and "predicate" is one of "inEffect" / "activatesProfile".
  #
  ...
  wrap = predicate: name: body: {
    ...
    config = lib.mkIf (predicate config.dotfiles._resolved name) (body args);
  };
...
in {
  ...
  # In addition to the per-class bodies that the "mkFeature" function
  # accepts, the "mkProfile" function recognizes one reserved key:
  #
