...
{lib}: let
  ...
  # "config.dotfiles._resolved" record.
  #
  ...
  wrap = name: body: {
    ...
    config = lib.mkIf (config.dotfiles._resolved.inEffect name) (body args);
  };
...
in {
  ...
  # whenever this one is — a bundle. Selecting the bundle activates
  # its members through the implication closure, the way a feature
  # bundle fans out to the finer ones; the edges join the
  # "impliedEdges" registry beside those a feature declares. Each
  # entry is a bare interest name — the record form that the
  # "mkFeature" function accepts for a platform-conditional edge has
  # no place here. An interest may imply only interests, never a
  # feature, which an assertion in the "modules/_assertions.nix" file
  # enforces at resolve time. A null or omitted value, or an empty
  # list, implies nothing.
  mkInterest = {
    name,
    description ? null,
    implies ? null,
  ...
  in
    lib.mkIf (lib.seq checks (lib.all config.dotfiles._resolved.inEffect names)) fragment;
}
