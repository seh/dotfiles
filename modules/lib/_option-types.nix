...
in {
  ...
  # meaningful value (say, "no forbidding" for the "forbiddenFeatures"
  ...
  setOfNames = {
    ...
    policy =
      policies.${merge}
      or (throw ''setOfNames: unknown "merge" policy "${merge}". The accepted values are "agreement", "union", and "intersection".'');
  in
