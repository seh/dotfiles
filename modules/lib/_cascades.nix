{lib}: let
  ...
  # declared "supportedPlatforms" (see the "mkProfile" function): a
  # profile the host's platform does not support never arrives through
  # the cascade, whether via the "all" entry or via a narrower one
  # such as "desktop". The assertion in "modules/_assertions.nix"
  # rejects a host that would activate such a profile anyway (e.g. by
  ...
  # downstream vertices in "excludedFeatures".
