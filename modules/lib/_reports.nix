...
{lib}: let
  ...
  # list that the evaluator's own "dotfiles._resolved.latentFeatures"
  ...
  evaluatorFor = {
    ...
    platform = evaluated.dotfiles._resolved.platform;
    withheld = evaluated.dotfiles._resolved.withheldFeatures;
    findings = findingsFor {
      ...
      inherit (evaluated.dotfiles._resolved) inEffect latentFeatures platform;
      inherit managed;
      ...
      obstructedNames = {
        forbidden = {
          feature = evaluated.dotfiles.host.forbiddenFeatures;
          interest = evaluated.dotfiles.host.forbiddenInterests;
        };
        ownExclusions = let
        ...
        in {
          feature =
            own evaluated.dotfiles._ownExcludedFeatures
            evaluated.dotfiles.host.excludedFeatures;
          interest =
            own evaluated.dotfiles._ownExcludedInterests
            evaluated.dotfiles.host.excludedInterests;
        };
        ...
        hostExclusions = {
          feature = evaluated.dotfiles.host.excludedFeatures;
          interest = evaluated.dotfiles.host.excludedInterests;
        };
      };
    };
  };
  ...
  evaluatorsWhenBuiltHere = collect: configuration:
    if
      ...
      && configuration.config.dotfiles ? _resolved
  ...
  # a feature is always "forbiddenFeatures" or "excludedFeatures", and
  ...
  findingLines = selectionPath: finding: let
    ...
    excludingList =
      ...
      then "dotfiles.host.excludedFeatures"
      else "${selectionPath}.excludedFeatures";
  in
    ["${finding.feature}:"]
    ++ indent (
      ...
      then [(field findingWidth "forbidden by" "dotfiles.host.forbiddenFeatures")]
  ...
  evaluatorLines = evaluator: let
    ...
    withheldLines = name: let
      ...
      cause =
        ...
        # their own to cite.
        then field withheldWidth "forbidden by" "dotfiles.host.forbiddenFeatures"
        else
          field withheldWidth "excluded by" (
            if entry.excludedBy == "own"
            then "${evaluator.selectionPath}.excludedFeatures"
            else "dotfiles.host.excludedFeatures"
          );
    in
      ["${name}:"] ++ indent [cause];
  in
  ...
  in
    # Terminate the last line, as a program writing to a terminal
    # does.
