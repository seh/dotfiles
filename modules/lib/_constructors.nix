...
# the machine's own configuration.
{
  ...
  #      pass through untouched—the "//" below leaves them alone—and
  #      prune every walk, so a user who selects a forbidden thing
  #      still does not receive it. That user's own evaluator carries
  #      a warning saying so, since forbidding is the one place where
