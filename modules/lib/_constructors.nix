...
# features and their exclusions (profiles, features, excludeProfiles,
# excludedFeatures) under "dotfiles.host". The latter arrangement lets
# the existing "dotfiles._resolved" derivation in
...
# inside the nested home-manager evaluator without modification.
{
  ...
  # activation with each user's, computed by the "_resolved" record in
  ...
  # so the existing "dotfiles._resolved" derivation logic (which reads
  # "config.dotfiles.host.{profiles,features,excludeProfiles,excludedFeatures}"
