...
# the machine's own configuration.
{
  ...
  forbidOptions = ["forbiddenFeatures" "forbiddenInterests"];

  # The selections a user writes that "forbiddenFeatures" or
  # "forbiddenInterests" overrules, one entry per pair of a selected
  # name and whichever option forbids it. Both of the user's own lists
  # are read together, since a name written under either enters the
  ...
  forbidCost = {
    ...
    ownExclusions = userCfg.excludedFeatures ++ userCfg.excludedInterests;
  ...
  # so the "dotfiles._resolved" record in the
  # "modules/_activation.nix" file resolves one coherent activation
  # for that user from the machine's selections and the user's
  # together: the machine provisions every user it manages, and each
  # user adds to that.
  #
  # Layering lets an exclusion hold in three ways, applied alike to
  # the machine's features and interests:
  #   1. The machine's "forbiddenFeatures" and "forbiddenInterests"
  #      lists pass through untouched—the "//" operator below leaves
  #      them alone—and prune every walk, so a user who asks for a
  ...
  #   2. The machine's "excludedFeatures" and "excludedInterests"
  #      lists withhold a name from what it provisions, yet a user who
  #      asks for that same name—selecting it directly, or selecting a
  #      bundle that implies it—drops it from the exclusions in force
  #      for that user, opting back in.
  ...
  multiUserPropagationModule = userDir: {config, ...}: let
    ...
    entailedBy = userCfg:
      selectionClosure (userCfg.features ++ userCfg.interests) (
        userCfg.excludedFeatures
        ++ userCfg.excludedInterests
        ++ host.forbiddenFeatures
        ++ host.forbiddenInterests
      );
  in {
    home-manager.users =
        ...
        dotfiles = {
          ...
          host =
            ...
            // {
              ...
              excludedFeatures =
                userCfg.excludedFeatures
                ++ lib.subtractLists entailed host.excludedFeatures;
              excludedInterests =
                userCfg.excludedInterests
                ++ lib.subtractLists entailed host.excludedInterests;
            };
        };
        warnings = let
          cost = forbidCost {
    ...
    assertions =
      lib.mapAttrsToList (
        userName: _: let
          ...
          lost =
            lib.subtractLists
            (evaluated.host.forbiddenFeatures ++ evaluated._machineForbidFeatures)
            host.forbiddenFeatures
            ++ lib.subtractLists
            (evaluated.host.forbiddenInterests ++ evaluated._machineForbidInterests)
            host.forbiddenInterests;
        in {
          assertion = lost == [];
          message = let
          ...
          in ''
            Resolving ${label}: the configuration of the user "${userName}" removed ${names} from what this machine forbids. The "dotfiles.host.forbiddenFeatures" and "dotfiles.host.forbiddenInterests" lists are the machine's, and no user may set them aside. Remove whatever line in that user's own configuration writes them. To decline a name the machine merely excludes, select it instead.
          '';
        }
      )
      config.dotfiles.users;
