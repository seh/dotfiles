...
# machine alone.
{
  ...
  # name, and selecting a contingent feature fails the build, so no
  # user could ever countermand that entry. A user excludes one in
  # that user's own list; a machine that wants one never to activate
  # forbids it instead.
  ...
  # feature fails the build and no implied edge may target one, so
  # every way in is closed and the exclusion applies absolutely while
  # sitting in the list that promises otherwise. A machine that wants
  # a contingent feature never to activate writes it in
  # "forbiddenFeatures", the list whose entries already apply
  # absolutely.
  #
  # Only the machine's "excludedFeatures" list is judged: the walk
  # consults each exclusion list for its own kind alone, so a
  # contingent feature's name in the "excludedInterests" list
  # withholds nothing, and the role-mismatch assertion alone objects
  # to that misfiling.
  #
