...
# machine alone.
{
  ...
  platform = config.dotfiles._resolved.platform;
  ...
  # record at the machine level; each one but "forbiddenFeatures" and
  # "forbiddenInterests" also has a per-user counterpart under the
  ...
  roles = [
    {
      name = "features";
      excludeName = "excludedFeatures";
      forbidName = "forbiddenFeatures";
      selected = host.features;
      excluded = host.excludedFeatures;
      forbidden = host.forbiddenFeatures;
      known = knownFeatures;
    ...
    {
      name = "interests";
      excludeName = "excludedInterests";
      forbidName = "forbiddenInterests";
      selected = host.interests;
      excluded = host.excludedInterests;
      forbidden = host.forbiddenInterests;
  ...
  # of its own. A machine-level entry in the "excludedFeatures" list
  # is a default an affected user overrides by selecting the same
  # name, and selecting a contingent feature is refused, so no user
  ...
  activeNames =
    config.dotfiles._resolved.activeFeatures
    ++ config.dotfiles._resolved.expressedInterests;
  ...
  # it from the "forbiddenFeatures" list; selecting a contingent
  # feature is refused and no implied edge may target one, so every
  # way in is closed and the exclusion holds absolutely while sitting
  # in the list that promises otherwise. A machine that wants a
  # contingent feature never to activate writes it in
  # "forbiddenFeatures", the list whose entries already hold
  # absolutely.
  #
  # Only the machine's "excludedFeatures" list is judged: the walk
  # holds each exclusion list to its own kind, so a contingent
  # feature's name in the "excludedInterests" list withholds nothing,
  ...
  excludedContingent =
    ...
    (lib.unique (builtins.filter (n: builtins.elem n contingentNames) host.excludedFeatures));
  contingentExclusionAssertion = {
    assertion = excludedContingent == [];
    message = ''
      Resolving ${hostLabel}: the "${hostOption "excludedFeatures"}"
      ...
      "${hostOption "forbiddenFeatures"}" list instead.
    '';
  };
