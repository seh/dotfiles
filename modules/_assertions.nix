...
# requiring each feature to reject unknown tags individually.
{
  ...
  assertions = let
    hostTags = config.dotfiles._resolved.tags;
  ...
  in [
    {
      assertion = unknown == [];
      message = ''
        Host "${toString config.dotfiles._resolved.name}" declares tag(s) that no imported feature module advertises: ${lib.concatStringsSep ", " unknown}.
