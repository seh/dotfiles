...
# tools. This module creates the necessary symlinks.
{
  flake.featureModules.nixOS.bazel-fhs = {
    ...
    # via "env -"). NixOS's bash has a compiled-in default "PATH"
    # value of "/no-such-path", so scripts that expect standard tools
    # like "mktemp" fail when Bazel runs them with a sanitized
    # environment.
