...
# "modules/home/default.nix" file demands that it stay empty.
{
  ...
  # exposed as the "dotfiles._resolved.platform" option below.
  pkgs ? null,
  ...
}: let
  ...
  # name in the "excludedInterests" or "forbiddenInterests" list (or
  # an interest's in the feature lists) prunes nothing, exactly as the
  ...
  exclusionsInForce =
    ofKind config.dotfiles._knownFeatures host.excludedFeatures
    ++ ofKind config.dotfiles._knownInterests host.excludedInterests;
  ...
  ownExcludedFeatures = ownOr config.dotfiles._ownExcludedFeatures host.excludedFeatures;
  ownExcludedInterests = ownOr config.dotfiles._ownExcludedInterests host.excludedInterests;
  ...
  # Fold what "host.forbiddenFeatures" and "host.forbiddenInterests"
  # forbid into this evaluator's own exclusions: both prune every walk
  # before the "resolveActivation" function, so a forbidden name
  # activates nowhere. No selection a user writes undoes it. The host
  # record conveys what they forbid and keeps it intact when the
  # propagation module in the "modules/lib/_constructors.nix" file
  # layers the machine's record with a user's, so each user's own home
  # environment respects it through this same logic, with no extra
  # wiring.
  withForbidden = excluded:
    excluded
    ++ ofKind config.dotfiles._knownFeatures
    (host.forbiddenFeatures ++ config.dotfiles._machineForbidFeatures)
    ++ ofKind config.dotfiles._knownInterests
    (host.forbiddenInterests ++ config.dotfiles._machineForbidInterests);
...
in {
  ...
  options.dotfiles = {
    host = mkOption {
        ...
        options = {
          ...
          excludedFeatures = mkOption {
            ...
            description = ''
              ...
              may undo is "forbiddenFeatures". A user may exclude a
              ...
              and points instead to "forbiddenFeatures", the list that
              withholds a name from every selector outright.
            '';
          };
          excludedInterests = mkOption {
            type = types.listOf types.str;
            default = [];
            description = ''
              ...
              does not undo the "forbiddenInterests" list.
            '';
          };
          forbiddenFeatures = mkOption {
            ...
            description = ''
              ...
              "excludedFeatures" list, which prunes only its author's
              own walk and, at the machine level, yields to a user's
              explicit selection.
            '';
          };
          forbiddenInterests = mkOption {
            type = setOfNames {merge = "union";};
            default = [];
            description = ''
              ...
              receives it—not even a user who selects that same name.
              Forbidding is the absolute counterpart to the
              "excludedInterests" list, which prunes only its author's
              own walk and, at the machine level, yields to a user's
              explicit selection.
            '';
          };
      ...
      description = ''
        ...
        "forbiddenFeatures"/"forbiddenInterests" lists. In a system
                ...
                options = {
                  ...
                  excludedBy = mkOption {
                    ...
                    description = ''
                      ...
                      "excludedFeatures" list this evaluator writes for
                      itself; "machine" means the machine's own
                      "excludedFeatures" list, arriving at a managed
                  ...
                  forbidden = mkOption {
                    type = types.bool;
                    description = ''
                      True when the machine-wide "forbiddenFeatures" list
                      holds this feature. Only that list counts: the
                      walk lets "forbiddenFeatures" forbid features alone
                      and "forbiddenInterests" forbid interests alone, so
                      a feature's name in the "forbiddenInterests" list
                      ...
                      "forbiddenFeatures" option merges its definitions
              ...
              types.submodule {
                options = {
                  ...
                  forbidden = mkOption {
                    type = types.bool;
                    description = ''
                      True when "host.forbiddenFeatures" forbids this
                      feature itself.
                    '';
                  };
                  beyond = mkOption {
        ...
        in {
          ...
          latentFeatures =
            lib.mapAttrs (name: preconditions: {
              ...
              # Only the "excludedFeatures" lists can hold a feature
              ...
              # reader can act on.
              excludedBy =
                if builtins.elem name ownExcludedFeatures
                then "own"
                else if builtins.elem name host.excludedFeatures
                then "machine"
                else null;
              forbidden = builtins.elem name host.forbiddenFeatures;
              unsupportedOn = let
          ...
          withheldFeatures = let
            ...
            isForbidden = name:
              builtins.elem name host.forbiddenFeatures
              || builtins.elem name config.dotfiles._machineForbidFeatures;
            # The withheld features an author's own lists hold. Each
            # of the rest arrives only through one of these, so citing
    ...
    # accepted as intended. What the machine forbids, as the
    # propagation module in the "modules/lib/_constructors.nix" file
    # writes it into each managed user's evaluator. The walk prunes
    # these beside the "dotfiles.host.forbiddenFeatures" and
    # "dotfiles.host.forbiddenInterests" lists, so forbidding survives
    # a module inside a user's own home configuration rewriting that
    # record. Both stay empty where nothing propagates them—the
    # machine's own evaluator, and a home configuration nobody else
    # builds—since there the two "host" lists are the only source and
    # their author is the one forbidding.
    #
    ...
    _machineForbidFeatures = mkOption {
      ...
      description = ''
        ...
        copy of the "dotfiles.host.forbiddenFeatures" list.
      '';
    };
    _machineForbidInterests = mkOption {
      ...
      description = ''
        ...
        user's evaluator, which the walk prunes beside that user's own
        copy of the "dotfiles.host.forbiddenInterests" list.
      '';
    };
    ...
    _ownExcludedFeatures = mkOption {
      ...
      description = ''
        ...
        against the ones a machine passes down to it. Null in an
        evaluator that writes its own "host.excludedFeatures" list
    ...
    _ownExcludedInterests = mkOption {
      type = types.nullOr (types.listOf types.str);
      default = null;
      internal = true;
      description = ''
        The interests this evaluator excludes on its own account, the
        companion to the "_ownExcludedFeatures" option and null in the
        same places.
      '';
    };
  ...
  config = let
    ...
    # and what it forbids: "excludedFeatures" against the known
    # features and "forbiddenFeatures", "excludedInterests" against
    # the known interests and "forbiddenInterests", matching the walk,
    # which holds each list to its own kind. A name written under the
    # wrong list is a kind mismatch, which the assertions in the
    ...
    redundantFeatures =
      ...
      (redundantOf config.dotfiles._knownFeatures host.forbiddenFeatures ownExcludedFeatures);
    redundantInterests =
      lib.subtractLists contradictoryInterests
      (redundantOf config.dotfiles._knownInterests host.forbiddenInterests ownExcludedInterests);
    # What the machine forbids, keyed by kind.
    forbidOf = {
      feature = host.forbiddenFeatures;
      interest = host.forbiddenInterests;
    };
    forbidOptionOf = {
      feature = "forbiddenFeatures";
      interest = "forbiddenInterests";
    };
    # The exclusion list each kind's own entries live in, for the
    # machine's-entry test below.
    ownExclusionsOf = {
      feature = ownExcludedFeatures;
      interest = ownExcludedInterests;
    };
    ...
    contradictoryFeatures = bothIn ownFeatures ownExcludedFeatures;
    contradictoryInterests = bothIn ownInterests ownExcludedInterests;
  ...
  in {
    ...
    warnings = lib.seq _unprunedSideEffect (
      map (mkContradiction "feature" "features" "excludedFeatures") contradictoryFeatures
      ++ map (mkContradiction "interest" "interests" "excludedInterests") contradictoryInterests
      ...
      ++ map (mkWarning "feature" "excludedFeatures") redundantFeatures
      ++ map (mkWarning "interest" "excludedInterests") redundantInterests
    );
  };
}
