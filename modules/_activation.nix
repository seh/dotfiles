...
# empty.
{
  ...
  # "excludedInterests" or "forbiddenInterests" (or an interest's in
  # the feature lists) prunes nothing, exactly as the complaint says
  # to expect. The profile lists need no such filter, because the
  ...
  # Fold what "host.forbidProfiles", "host.forbiddenFeatures", and
  # "host.forbiddenInterests" forbid into this evaluator's own
...
in {
  ...
  options.dotfiles = {
    host = mkOption {
      ...
      description = ''
        ...
        "forbidProfiles"/"forbiddenFeatures"/"forbiddenInterests" lists. In
  ...
  # (or "excludedFeatures", or "excludedInterests"), recompute this
  ...
  config = let
    ...
    redundantFeatures = redundantOf "features" config.dotfiles._knownFeatures host.excludedFeatures;
    redundantInterests = redundantOf "features" config.dotfiles._knownInterests host.excludedInterests;
