...
# "modules/home/default.nix" demands that it stay empty.
{
  ...
  ownExcludedFeatures =
    if config.dotfiles._ownExcludedFeatures != null
    then config.dotfiles._ownExcludedFeatures
    else host.excludedFeatures;
  ownExcludedInterests =
    if config.dotfiles._ownExcludedInterests != null
    then config.dotfiles._ownExcludedInterests
    else host.excludedInterests;
...
in {
        ...
        options = {
          ...
          excludedFeatures = mkOption {
            ...
            description = ''
              ...
              "forbiddenFeatures". A user may exclude a contingent
              ...
              points instead to "forbiddenFeatures", the list that
  ...
  # may be removed. For each candidate name "n" in "excludedFeatures"
  # (or "excludedInterests"), recompute this evaluator's own
  # activation with "n" temporarily removed from its exclusion list
  # (but with all other exclusions still pruning the graph). If "n" is
  # absent from the resulting activation, it would not have been
  # active anyway, so listing it as excluded changes nothing. The
  # judgment stays within one evaluator: the machine's own selections
  # in a system evaluator, and the machine's layered with the user's
  # in a managed user's evaluator, so each evaluator reports only the
  # exclusions idle there.
  config = let
    ...
    # "excludedFeatures" against the known features and
    # "excludedInterests" against the known interests, matching the
    ...
    redundantFeatures = redundantOf config.dotfiles._knownFeatures host.excludedFeatures;
    redundantInterests = redundantOf config.dotfiles._knownInterests host.excludedInterests;
