...
flakeLib.mkFeature "editor/emacs" {
  homeManager = {
    ...
    config = {
      ...
      inherit (config.dotfiles._resolved) inEffect;
      diaryFile = "~/.diary";
      quoteAll = names: lib.concatMapStringsSep "\n    " (name: ''"${name}"'') (lib.sort lib.lessThan names);

      # The activation names the flake registers and the ones in
      # effect for this home environment, as an Emacs Lisp file that
      # the "activation.el" file consults.
      activationNames = pkgs.writeText "activation-names.el" ''
        ...
        (defconst seh-activation-names-in-effect
          '(${quoteAll (config.dotfiles._resolved.activeFeatures ++ config.dotfiles._resolved.expressedInterests)})
      ...
      # Byte-compile the Emacs Lisp files with the Emacs that Home
      # Manager installs, so every package is on the compiler's load
      # path.
      emacsDirectory =
        pkgs.runCommand "emacs.d" {
          ...
          cd "$out"
          # Some packages read or write below the home directory as
          # they load. Nix sets the "HOME" environment variable to a
          # path that does not exist, so create a writable directory
          # and point that variable at it for the compilation.
          export HOME="$NIX_BUILD_TOP/home"
          mkdir -p "$HOME"
          # Compile each file in its own Emacs process, so definitions
          # one file leaves loaded cannot hide warnings in the next.
          for file in ./*.el; do
            emacs --batch \
    ...
    in {
      ...
      # set, displaying a calendar reads that file, and Emacs signals
      # an error when it is absent. Create an empty one, and leave an
      # existing file alone.
      ...
      programs.emacs = {
        ...
        # C-M-SPC key binding for "mark-sexy" gets intercepted by
        # macOS and presents the Character Viewer applet. package =
        ...
        extraPackages = epkgs:
          with epkgs;
            [
              ...
              bbdb
              beacon
              # An autoload cookie above a macro call copies the call
              # into the generated autoloads file, where Emacs cannot
              # expand it yet; the patch autoloads the command
              # explicitly instead.
