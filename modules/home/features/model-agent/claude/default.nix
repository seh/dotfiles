...
flakeLib.mkFeature "model-agent/claude" {
  homeManager = {
    ...
    enabledPluginIDs =
      lib.optionals (config.dotfiles._resolved.inEffect "dev/language-servers") [
        "gopls-lsp@claude-plugins-official"
        "typescript-lsp@claude-plugins-official"
      ]
      ++ lib.optional (config.dotfiles._resolved.inEffect "lang/rust") "rust-analyzer-lsp@claude-plugins-official";
  in {
    programs.claude-code = {
      ...
      lspServers = lib.mkMerge [
        ...
        (lib.mkIf (config.dotfiles._resolved.inEffect "lang/lua") {
          lua = {
        ...
        (lib.mkIf (config.dotfiles._resolved.inEffect "dev/language-servers") {
          bash = {
          ...
          hooks = {
            PostToolUse = let
              ...
              languages =
                ...
                ++ lib.optional (config.dotfiles._resolved.inEffect "lang/lua") {
                  # Lua files
                  patterns = ["*.lua"];
                  command = ''${lib.getExe pkgs.stylua} --config-path ${config.dotfiles.lua.styluaConfigFile} -- ${receiveInputFilePath}'';
                }
                ++ lib.optional (config.dotfiles._resolved.inEffect "lang/markdown") {
                  # Markdown files
                  patterns = ["*.md"];
