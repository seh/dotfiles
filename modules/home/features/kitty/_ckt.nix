...
# imports it explicitly.
{
  ...
  # supplied here and writes to the path that Nix supplies in the
  # "out" variable.
  ...
  # here rather than on each cursor movement means the printing
  # program handles no color syntax, and so cannot misread one; kitty
  # itself reads the colors, so every form kitty accepts is drawn
  # correctly.
...
in
  pkgs.writeShellApplication {
    ...
    runtimeInputs = [
      ...
      pkgs.sqlite
      # Supplies the "kitten" tool, with which the program controls
      # the running kitty instance.
      kittyPackage
    ];
    # Substituting these paths here leaves the program file free of
    # Nix syntax, so that the "shellcheck" and "shfmt" tools read it
    # as an ordinary bash program. Each substitution preserves its
    # store reference, so the "kitty-themes" package, the list of
    # theme names, the two programs above and the schema all become
