{
  ...
  in {
    config = mkIf (config.dotfiles._resolved.activatesProfile "essential") {
      home = {
        packages = with pkgs;
          [
            ...
            # use with the "nix check" command, but that configuraton
            # does not link the "alejandra" program provided by this
            # package from a directory on our path along with these
            # other tools.
            alejandra
            btop
            coreutils
