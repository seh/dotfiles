  ...
  in {
    config = mkMerge [
      ...
      (mkIf (hasTag "macos") {
        ...
        targets.darwin = {
          # TODO(SEHarris): Remove these once all machines are using
          # Home Manager's state version 25.11 or newer.
          copyApps.enable = mkDefault true;
          linkApps.enable = mkDefault false;

          defaults = {
            "com.apple.dock" = {
              # System Preferences > Mission Control > Group windows
              # by application
              expose-group-apps = mkDefault true;
              # System Preferences > Dock > Position on screen
              orientation = mkDefault "right";
              # System Preferences > Dock > Show recent applications
              # in Dock
