...
#   https://github.com/midchildan/dotfiles/blob/8105b21f1a743960a8bbbf5bdcd752a7d9b60d10/nix/home/profiles/fonts.nix
{
  ...
  in {
    config = lib.mkMerge [
      {
        dotfiles._knownTags = ["fonts"];
      }
      (lib.mkIf (hasTag "fonts") {
        fonts.fontconfig.enable = lib.mkDefault isLinux;

        home.packages = with pkgs; [
          # NB: The "powerline-fonts" package contains DejaVu Sans
          # Mono, Iconsolata, and Meslo. However, there are additional
          # fonts in the DejaVu family, so we'll pull them in as well.
          #
          # Excluded:
          # - inconsolata
          # - meslo-lgs-nf
